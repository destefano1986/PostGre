...
sql = "SELECT n.*, r.* FROM (SELECT * FROM pm_eutrancelltdd_day WHERE datetime = '2018-09-10 00:00:00') AS n, (SELECT * FROM resource_eutrancell WHERE date = '2018-09-10') as r WHERE n.dn=r.dn"
cursor.execute(sql)
title = [i[0] for i in cursor.description]
data = cursor.fetchall()
df = pd.DataFrame(data, columns = title)
df.to_csv('pm_eutrancelltdd_day_0911.csv', index=True, header=True, encoding='gbk')
#print (data)
cursor.close()
conn.close()