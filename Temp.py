...
#sql = "SELECT dn, userlabel, pdcpupoctul, pdcpupoctdl, rrutotalprbusagemeanul, rrutotalprbusagemeandl FROM pm_eutrancelltdd_day WHERE datetime = '2018-07-01 00:00:00'"
...
#sql = "SELECT n.dn, n.userlabel, n.pdcpupoctul, n.pdcpupoctdl, r.* FROM (SELECT dn, userlabel, pdcpupoctul, pdcpupoctdl FROM pm_eutrancelltdd_day WHERE datetime = '2018-07-26 00:00:00') AS n, (SELECT * FROM resource_eutrancell WHERE date = '2018-07-26') as r WHERE n.dn=r.dn"
cursor.execute(sql)
title = [i[0] for i in cursor.description]
data = cursor.fetchall()
df = pd.DataFrame(data, columns = title)
df.to_csv('pm_eutrancelltdd_day_0726_2.csv', index=True, header=True, encoding='gbk')
#print (data)
cursor.close()
conn.close()
